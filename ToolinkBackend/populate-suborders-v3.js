...
// Helper function to generate sub-order number in correct format
const generateSubOrderNumber = (mainOrderNumber, warehouseCode, sequence) => {
    // Convert main order to correct format first
    const formattedMainOrder = convertOrderNumberFormat(mainOrderNumber);
    const sequenceStr = sequence.toString().padStart(3, '0');
    return `${formattedMainOrder}-${warehouseCode}-${sequenceStr}`;
};

const populateSubOrdersV3 = async () => {
    console.log('🚀 Starting SubOrders population from main Orders (v3)...');
    ...
    try {
        ...
        // Test order number format conversion
        if (orders.length > 0) {
            const sampleOrder = orders[0];
            const convertedNumber = convertOrderNumberFormat(sampleOrder.orderNumber);
            console.log(`📝 Order number format test:`);
            console.log(`  Original: ${sampleOrder.orderNumber}`);
        ...
        for (const order of orders) {
            try {
                ...
                for (const [warehouseCode, warehouseItems] of Object.entries(warehouseGroups)) {
                    ...
                    const mainOrderNumber = convertOrderNumberFormat(order.orderNumber);

                    console.log(`    📋 Sub-order number: ${subOrderNumber}`);
                    console.log(`    📋 Main order number: ${mainOrderNumber}`);
        ...
        const sampleSubOrders = await SubOrder.find({}).limit(5).populate('warehouseId');
        ...
        if (sampleSubOrders.length > 0) {
            ...
            sampleSubOrders.forEach(so => {
                ...
                console.log(`    Warehouse: ${so.warehouseId?.name || 'Unknown'} (${so.warehouseCode})`);
